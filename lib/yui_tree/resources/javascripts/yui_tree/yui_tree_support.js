...
function uk_org_pond_yui_tree_support( options )
{
  this.onPageLoad = function()
  {
    ...
    options.nameFieldBlank = options.nameFieldBlank || ''
    ...
    if ( nameField && ( ! options.highlight || options.highlight.length == 0 ) )
    {
      nameField.innerHTML = options.nameFieldBlank;
    }
    ...
    function nodeHighlightChanged( node )
    {
      if ( formField )
      {
        ...
        if ( nameField )
        {
          ...
          if ( text == '' ) text = options.nameFieldBlank;
